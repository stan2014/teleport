...
func (f *rdsDocumentDBFetcher) GetDatabases(ctx context.Context, cfg *awsFetcherConfig) (types.Databases, error) {
	...
	for _, cluster := range clusters {
		if !services.IsDocumentDBClusterSupported(cluster) {
		...
		if !services.IsDocumentDBClusterAvailable(cluster.Status, cluster.DBClusterIdentifier) {
		...
		dbs, err := services.NewDatabasesFromDocumentDBCluster(cluster)
		if err != nil {
