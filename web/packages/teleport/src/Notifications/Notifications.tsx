...
export function Notifications({ iconSize = 24 }: { iconSize?: number }) {
  ...
  function updateLocalNotificationsState() {
    setLocalNotifications(
      store.getNotifications().map(notif => {
  ...
  useEffect(() => {
    ...
    store.subscribe(updateLocalNotificationsState);

    return () => {
      store.unsubscribe(updateLocalNotificationsState);
    };
  }, []);
  ...
  function onIconClick() {
    if (!open) {
      ...
      if (combinedNotifications.length) {
        const latestNotificationTime = combinedNotifications[0].createdDate;
  ...
  const unseenNotifsCount = combinedNotifications.filter(
    notif =>
