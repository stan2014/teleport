...
**Teleport agents** route traffic from users through the Teleport Proxy Service
to and from resources in your infrastructure. This guide describes the
architecture that enables Teleport to securely manage traffic through this
pathway.
...
manage agents, including on cloud-hosted Teleport Enterprise accounts. 
...
{/* TODO: Expand this paragraph to talk about the contents of the X.509 certs
issued for mTLS between the Proxy Service and agents*/}
...
Agents use the reverse tunnel to retrieve certificates from the Teleport Auth
Service. {/*TODO: How does authentication work when first setting up the reverse
tunnel?*/}

{/* TODO: Expand on this paragraph. The agent uses a cert to authenticate with
the Proxy Service? What does it mean to route a connection to the appropriate
agent?*/}
The connection between the Proxy Service and a Teleport agent instance is also
authenticated with mutual TLS. The Proxy Service authenticates the connection
and dispatches it to the appropriate agent instance based on the routing
information encoded in the client certificate, over the reverse tunnel.
