...
authenticated with mutual TLS. The Auth Service generates a short-lived X.509
certificate signed by the cluster's host CA, with the client's identity and
routing information for an infrastructure resource encoded in it, and the Proxy
Service uses it to authenticate with the Teleport agent.
...
## Agents and infrastructure resources
