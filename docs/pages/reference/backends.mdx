...
teleport:
  storage:
    ...
    # GCP connection name in format of <project>:<location>:<instance>.
    ...
    audit_events_uri:
      - postgresql://<Var name="service-account@project.iam"/>@/teleport_audit#auth_mode=gcp-sql&gcp_connection_name=<Var name="project:location:instance"/>
```

To enable IAM authentication and logical replication for Cloud SQL, make sure
flag `cloudsql.iam_authentication` and flag `cloudsql.logical_decoding` and are
set to `on` for the Cloud SQL instance. The database user must also have the
...
In order for Teleport to use [Cloud SQL Go
Connector](https://cloud.google.com/sql/docs/mysql/connect-connectors) with IAM
authentication, the service account of the target databse user must have "Cloud
...
Teleport will make use of the credentials specified through
